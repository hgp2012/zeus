...
public class WorkerBeExecute {
	...
	public Future<Response> manual(final WorkerContext context,
		...
		SocketLog.info("WorkerBeExecute receive master to worker manual request,rid="
				...
				new Callable<Response>() {
					public Response call() throws Exception {
						...
						Response resp = Response.newBuilder().setRid(
								...
								.build();
						SocketLog
								.info("WorkerBeExecute send manual response,manual complete,rid="
										+ req.getRid()
										+ ",historyId="
										+ historyId);
						return resp;
	...
	public Future<Response> debug(final WorkerContext context, final Request req) {
		...
		SocketLog.info("WorkerBeExecute receive master to worker debug request,rid="
	...
	private Future<Response> schedule(final WorkerContext context,
		...
		SocketLog.info("WorkerBeExecute receive master to worker execute request,rid="
				...
				new Callable<Response>() {
					public Response call() throws Exception {
						...
						Response resp = Response.newBuilder().setRid(
								...
								.build();
						SocketLog
								.info("WorkerBeExecute send execute response,execute complete,rid="
										+ req.getRid()
										+ ",jobId="
										+ history.getJobId());
						return resp;

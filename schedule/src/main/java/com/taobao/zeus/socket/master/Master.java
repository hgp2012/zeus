...
public class Master {
	...
	public Master(final MasterContext context) {
								...
								while (itController.hasNext()) {
									...
									if (Long.parseLong(jobId) < (Long.parseLong(currentDateStr) - 15000000)) {
										try {
											...
											&& Long.parseLong(jobId) < Long.parseLong(nextDateStr)) {
										try {
		...
		context.getSchedulePool().scheduleAtFixedRate(new Runnable() {
			...
			public void run() {
				try {
					...
					if ((execHour == 0 && execMinute == 0) || (execHour == 0 && execMinute == 35)
								...
								for (Long id : actionDetailsNew.keySet()) {
									...
									if (id > Long.parseLong(currentDateStr)) {
										context.getDispatcher()
												.forwardEvent(new JobMaintenanceEvent(Events.UpdateJob, id.toString()));
									}
								}
							}
						}
		...
		context.getSchedulePool().scheduleAtFixedRate(new Runnable() {
			...
			public void run() {
				Date now = new Date();
				for (MasterWorkerHolder holder : new ArrayList<MasterWorkerHolder>(context.getWorkers().values())) {
					// log.info("schedule worker
					// start:"+holder.getDebugRunnings().size());
					try {
						if (holder.getHeart().timestamp == null
								|| (now.getTime() - holder.getHeart().timestamp.getTime()) > 1000 * 60) {
					...
					// log.info("schedule worker
					// end:"+holder.getDebugRunnings().size());
				}
			}
				...
				if (jobDependStr != null && jobDependStr.trim().length() > 0) {
					...
					if (isAllComplete) {
						if (!rollBackActionId.contains(id)) {
							...
							// System.out.println("roll back lost jobID :" +
							// id.toString());
							// log.info("roll back lost jobID :" +
							// id.toString());
						}
					}
				} else {
	...
	// if (heart != null && heart.memRate != null && heart.memRate <
	// HostIndex.MAX_MEM_RATE && heart.cpuLoadPerCore <
	// HostIndex.MAX_CPULOAD_PER_CORE) {
	...
	// for (MasterWorkerHolder worker : context.getWorkers().values()) {
	// try {
	// if (worker!=null && worker.getHeart()!=null &&
	// workersGroup.contains(worker.getHeart().host)) {
	// HeartBeatInfo heart = worker.getHeart();
	// if (heart != null && heart.memRate != null && heart.memRate <
	// Environment.getMaxMemRate() && heart.cpuLoadPerCore <
	// Environment.getMaxCpuLoadPerCore() ) {
	// if (selectWorker == null) {
	// selectWorker = worker;
	// selectMemRate = heart.memRate;
	...
	// }
	// if (selectWorker != null) {
	// log.info("select worker: " + selectWorker.getHeart().host + ", for
	...
	public synchronized MasterWorkerHolder getRunableWorker(String hostGroupId) {
		if (hostGroupId == null) {
			hostGroupId = Environment.getDefaultWorkerGroupId();
		}
		MasterWorkerHolder selectWorker = null;
	...
	private void scan() {
		...
		if (!context.getManualQueue().isEmpty()) {
			...
			if (selectWorker == null) {
				...
				log.info("HostGroupId : " + e.getHostGroupId() + ",schedule selectWorker : " + selectWorker + ",host :"
						+ selectWorker.getHeart().host);
			}
		}
	...
	private void runDebugJob(MasterWorkerHolder selectWorker, final String jobID) {
		...
		SocketLog.info("master scan and poll debugId=" + jobID + " and run!");

		new Thread() {
			@Override
			public void run() {
				DebugHistory history = context.getDebugHistoryManager().findDebugHistory(jobID);
				history.getLog().appendZeus(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + " 开始运行");
				context.getDebugHistoryManager().updateDebugHistoryLog(jobID, history.getLog().getContent());
				Exception exception = null;
				Response resp = null;
				try {
					Future<Response> f = new MasterExecuteJob().executeJob(context, w, ExecuteKind.DebugKind, jobID);
					resp = f.get();
				} catch (Exception e) {
					exception = e;
					DebugInfoLog.error(String.format("debugId:%s run failed", jobID), e);
				}
				...
				if (!success) {
					// 运行失败，更新失败状态，发出失败消息
					if (exception != null) {
						exception = new ZeusException(String.format("fileId:%s run failed ", history.getFileId()),
								exception);
					} else {
						exception = new ZeusException(String.format("fileId:%s run failed ", history.getFileId()));
					}
					DebugInfoLog.info("debugId:" + jobID + " run fail ");
					history = context.getDebugHistoryManager().findDebugHistory(jobID);
					DebugFailEvent jfe = new DebugFailEvent(history.getFileId(), history, exception);
					context.getDispatcher().forwardEvent(jfe);
				} else {
					// 运行成功，发出成功消息
					DebugInfoLog.info("debugId:" + jobID + " run success");
					DebugSuccessEvent dse = new DebugSuccessEvent(history.getFileId(), history);
	...
	public void runManualJob(MasterWorkerHolder selectWorker, final String jobID) {
		final MasterWorkerHolder w = selectWorker;
		// final JobElement historyId = context.getManualQueue().poll();
		SocketLog.info("master scan and poll historyId=" + jobID + " and run!");
		new Thread() {
			@Override
			public void run() {
				JobHistory history = context.getJobHistoryManager().findJobHistory(jobID);
				
				history.getLog().appendZeus(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + " 开始运行");
				context.getJobHistoryManager().updateJobHistoryLog(jobID, history.getLog().getContent());
				// 更新手动运行状态
				...
				Response resp = null;
				try {
					Future<Response> f = new MasterExecuteJob().executeJob(context, w, ExecuteKind.ManualKind, jobID);
					resp = f.get();
				} catch (Exception e) {
					exception = e;
					ScheduleInfoLog.error("JobId:" + history.getJobId() + " run failed", e);
				...
				if (!success) {
					...
					if (exception != null) {
						jobException = new ZeusJobException(history.getJobId(),
								String.format("JobId:%s run failed ", history.getJobId()), exception);
					} else {
						jobException = new ZeusJobException(history.getJobId(),
								String.format("JobId:%s run failed ", history.getJobId()));
					}
					ScheduleInfoLog.info("jobId:" + history.getJobId() + " run fail ");
					history = context.getJobHistoryManager().findJobHistory(jobID);

					jobstatus.setStatus(com.taobao.zeus.model.JobStatus.Status.FAILED);
					JobFailedEvent jfe = new JobFailedEvent(history.getJobId(), history.getTriggerType(), history,
							jobException);
					if (history.getIllustrate() == null || !history.getIllustrate().contains("手动取消该任务")) {
						context.getDispatcher().forwardEvent(jfe);
					}
				} else {
					// 运行成功，发出成功消息
					ScheduleInfoLog.info("manual jobId::" + history.getJobId() + " run success");
					jobstatus.setStatus(com.taobao.zeus.model.JobStatus.Status.SUCCESS);
					JobSuccessEvent jse = new JobSuccessEvent(history.getJobId(), history.getTriggerType(), jobID);
			...
			public void run() {
				...
				try {
					...
					if (properties != null && properties.size() > 0) {
						rollBackTimes = Integer.parseInt(
								properties.get("roll.back.times") == null ? "0" : properties.get("roll.back.times"));
						rollBackWaitTime = Integer.parseInt(properties.get("roll.back.wait.time") == null ? "1"
								: properties.get("roll.back.wait.time"));
					}
				} catch (Exception ex) {
					rollBackTimes = 0;
					rollBackWaitTime = 1;
	...
	// schedule任务运行，失败后重试
	private void runScheduleJobContext(MasterWorkerHolder w, final String jobID, int runCount, final int rollBackTimes,
			final int rollBackWaitTime) {
		runCount++;
		...
		if (runCount == 1) {
			his = context.getJobHistoryManager()
					.findJobHistory(context.getGroupManager().getJobStatus(jobID).getHistoryId());
			type = his.getTriggerType();
			ScheduleInfoLog.info("JobId:" + jobID + " run start");
			his.getLog().appendZeus(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + " 开始运行");
		} else {
			...
			his.getLog().appendZeus(
					new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + " 第" + (runCount - 1) + "次重试运行");
		}
		...
		Response resp = null;
		try {
			Future<Response> f = new MasterExecuteJob().executeJob(context, w, ExecuteKind.ScheduleKind, his.getId());
			resp = f.get();
		} catch (Exception e) {
			exception = e;
			ScheduleInfoLog.error(String.format("JobId:%s run failed", jobID), e);
			jobstatus.setStatus(com.taobao.zeus.model.JobStatus.Status.FAILED);
			context.getGroupManager().updateJobStatus(jobstatus);
		}
		boolean success = resp.getStatus() == Status.OK ? true : false;
		if (success && (his.getTriggerType() == TriggerType.SCHEDULE
				|| his.getTriggerType() == TriggerType.MANUAL_RECOVER)) {
		...
		if (!success) {
			...
			ZeusJobException jobException = null;
			if (exception != null) {
				jobException = new ZeusJobException(jobID, String.format("JobId:%s run failed ", jobID), exception);
			} else {
				jobException = new ZeusJobException(jobID, String.format("JobId:%s run failed ", jobID));
			}
			...
			jobstatus.setStatus(com.taobao.zeus.model.JobStatus.Status.SUCCESS);
			JobSuccessEvent jse = new JobSuccessEvent(jobID, his.getTriggerType(), his.getId());
	...
	private void checkDebugTimeOver(MasterWorkerHolder w) {
		for (Map.Entry<String, Boolean> entry : w.getDebugRunnings().entrySet()) {
			...
			DebugHistory his = context.getDebugHistoryManager().findDebugHistory(historyId);
			long maxTime;
			FileDescriptor fd;
			try {
				fd = context.getFileManager().getFile(his.getFileId());
				Profile pf = context.getProfileManager().findByUid(fd.getOwner());
				String maxTimeString = pf.getHadoopConf().get("zeus.job.maxtime");
	...
	private void checkManualTimeOver(MasterWorkerHolder w) {
		for (Map.Entry<String, Boolean> entry : w.getManualRunnings().entrySet()) {
			if (entry.getValue() != null && entry.getValue()) {
				continue;
			}
			String historyId = entry.getKey();
			JobHistory his = context.getJobHistoryManager().findJobHistory(historyId);
			JobDescriptor jd = context.getGroupManager().getJobDescriptor(his.getJobId()).getX();
			long maxTime;
			try {
	...
	private void checkScheduleTimeOver(MasterWorkerHolder w) {
		for (Map.Entry<String, Boolean> entry : w.getRunnings().entrySet()) {
			...
			JobHistory his = context.getJobHistoryManager()
					.findJobHistory(context.getGroupManager().getJobStatus(jobId).getHistoryId());
	...
	private boolean timeOverAlarm(final JobHistory his, FileDescriptor fd, long runTime, long maxTime, int type,
			JobDescriptor jd) {
		final MailAlarm mailAlarm = (MailAlarm) context.getApplicationContext().getBean("mailAlarm");
		SMSAlarm smsAlarm = (SMSAlarm) context.getApplicationContext().getBean("smsAlarm");
		...
		content.append("\n已经运行时间：").append(runTime).append("分钟").append("\n设置最大运行时间：").append(maxTime).append("分钟");
		if (his != null) {
			...
			if (type == 2) {
				...
				new Thread() {
					...
					public void run() {
						try {
							Thread.sleep(6000);
							mailAlarm.alarm(his.getId(), title.toString(), content.toString().replace("\n", "<br/>"));
						} catch (Exception e) {
				...
				if (type == 0) {
					...
					if (priorityLevel == null || !priorityLevel.trim().equals("1")) {
						...
						if (day == Calendar.SATURDAY || day == Calendar.SUNDAY || hour < 9 || hour > 18) {
							...
							// mailAlarm.alarm(his.getId(),
							// title.toString(),content.toString(), null);
						}
					}
				}
			}
	...
	public void workerDisconnectProcess(Channel channel) {
		...
		if (holder != null) {
			// SocketLog.info("worker disconnect, ip:" +
			// channel.getRemoteAddress().toString());
			context.getWorkers().remove(channel);
			final List<JobHistory> hiss = new ArrayList<JobHistory>();
			Map<String, Tuple<JobDescriptor, JobStatus>> map = context.getGroupManager()
					.getJobDescriptor(holder.getRunnings().keySet());
			for (String key : map.keySet()) {
				JobStatus js = map.get(key).getY();
				if (js.getHistoryId() != null) {
					JobHistory his = context.getJobHistoryManager().findJobHistory(js.getHistoryId());
					if (his != null) {
						hiss.add(his);
					}
				}
	...
	public void debug(DebugHistory debug) {
		...
		debug.getLog().appendZeus(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + " 进入任务队列");
		context.getDebugHistoryManager().updateDebugHistoryLog(debug.getId(), debug.getLog().getContent());
	...
	public JobHistory run(JobHistory history) {
		...
		if (history.getTriggerType() == TriggerType.MANUAL_RECOVER || history.getTriggerType() == TriggerType.MANUAL_DEPENDENT) {
		...
		if (history.getStatus() == com.taobao.zeus.model.JobStatus.Status.RUNNING) {
			...
			history.getLog().appendZeus(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + " 进入任务队列");
			...
			} else if(history.getTriggerType() == TriggerType.MANUAL_DEPENDENT){//自定义新增   指定任务及依赖任务执行   页面点击响应
			
				/**
				 * 获取所有依赖执行的actionId，并且添加到Dispatcher的 Controller缓存
				 * 每次任务支持执行完后，会到Controller缓存查询是否有依赖的任务要执行
				 */
				List<String> dependencieds = context.getGroupManagerOld().getAllDependencied(history.getToJobId());
				dependencieds.add(history.getToJobId());
			
				Long id_pr = Long.parseLong(history.getJobId())- Long.parseLong(history.getToJobId());
				
				Dispatcher dispatcher = context.getDispatcher();
				for(int i=0;i<dependencieds.size();i++){
					String actionId = String.valueOf(id_pr+Integer.parseInt(dependencieds.get(i)));
					dispatcher.addController(new JobController(context, context.getMaster(), actionId));					
				}
				
				
				
				JobStatus js = context.getGroupManager().getJobStatus(history.getJobId());
	...
	public void runScheduleJobToAction(List<JobPersistenceOld> jobDetails, Date now, SimpleDateFormat dfDate,
		...
		for (JobPersistenceOld jobDetail : jobDetails) {
			...
			if (jobDetail.getScheduleType() != null && jobDetail.getScheduleType() == 0) {
				try {
					...
					if (jobCronExpression != null && jobCronExpression.trim().length() > 0) {
						...
						for (int i = 0; i < lTime.size(); i++) {
							String actionDateStr = ZeusDateTool.StringToDateStr(lTime.get(i), "yyyy-MM-dd HH:mm:ss",
									"yyyyMMddHHmm");
							String actionCronExpr = ZeusDateTool.StringToDateStr(lTime.get(i), "yyyy-MM-dd HH:mm:ss",
									"0 m H d M") + " ?";

							JobPersistence actionPer = new JobPersistence();
							actionPer.setId(Long.parseLong(actionDateStr) * 1000000 + jobDetail.getId());// update
																											// action
																											// id
							actionPer.setToJobId(jobDetail.getId());
							actionPer.setAuto(jobDetail.getAuto());
							actionPer.setConfigs(jobDetail.getConfigs());
							actionPer.setCronExpression(actionCronExpr);// update
																		// action
																		// cron
																		// expression
				 ...
				 * 取消生成周期任务 if(jobDetail.getScheduleType() != null &&
				 * jobDetail.getScheduleType()==2){ try{
				 * if(jobDetail.getDependencies()==null ||
				 * jobDetail.getDependencies().trim().length()==0){ Date date =
				 * null; try { date =
				 * DateUtil.timestamp2Date(jobDetail.getStartTimestamp(),
				 * DateUtil.getDefaultTZStr()); //System.out.println(date); }
				 * catch (ParseException e) { date = new Date(); log.error(
				 * "parse job start timestamp to date failed,", e); }
				 * SimpleDateFormat dfTime=new SimpleDateFormat("HHmmss");
				 * SimpleDateFormat dfDate=new SimpleDateFormat("yyyyMMdd");
				 * SimpleDateFormat dfMinute=new SimpleDateFormat("mmss");
				 * String currentDate = dfDate.format(new Date()); String
				 * startTime = dfTime.format(date); String startMinute =
				 * dfMinute.format(date); //天-周期调度
				 * if(jobDetail.getCycle().equals("day")){ Date newStartDate =
				 * ZeusDateTool.StringToDate(currentDate+startTime,
				 * "yyyyMMddHHmmss"); Calendar calendar =
				 * Calendar.getInstance();
				 * calendar.setTimeInMillis(newStartDate.getTime());
				 * calendar.add(Calendar.DATE, -1); calendar.getTime(); Date
				 * statisStartTime = calendar.getTime();
				 * calendar.add(Calendar.MINUTE, 60*23+59); Date statisEndTime =
				 * calendar.getTime();
				 * 
				 * JobPersistence actionPer = new JobPersistence();
				 * actionPer.setId(Long.parseLong(currentDate+startTime)*10000+
				 * jobDetail.getId());//update action id
				 * actionPer.setToJobId(jobDetail.getId());
				 * actionPer.setAuto(jobDetail.getAuto());
				 * actionPer.setConfigs(jobDetail.getConfigs());
				 * actionPer.setCronExpression(jobDetail.getCronExpression());
				 * actionPer.setCycle(jobDetail.getCycle()); String
				 * jobDependencies = jobDetail.getDependencies();
				 * actionPer.setDependencies(jobDependencies);
				 * actionPer.setJobDependencies(jobDependencies);
				 * actionPer.setDescr(jobDetail.getDescr());
				 * actionPer.setGmtCreate(jobDetail.getGmtCreate());
				 ...
				 * actionPer.setStatus(jobDetail.getStatus());
				 * actionPer.setHostGroupId(jobDetail.getHostGroupId());
				 * actionPer.setTimezone(jobDetail.getTimezone()); try {
				 * //System.out.println("周期任务（天）JobId: " + jobDetail.getId()+
				 ...
				 * actionDetails.put(actionPer.getId(),actionPer); //}
				 * 
				 * } catch (ZeusException e) { System.out.println("failed");
				 * log.error("周期任务（天）JobId:" + jobDetail.getId() + " 生成Action"
				 * +actionPer.getId() + "失败", e); } }
				 * if(jobDetail.getCycle().equals("hour")){ for (int i = 0; i <
				 * 24; i++) { String startHour = String.valueOf(i);
				 * if(startHour.trim().length()<2){ startHour = "0"+startHour; }
				 * Date newStartDate =
				 * ZeusDateTool.StringToDate(currentDate+startHour+startMinute,
				 * "yyyyMMddHHmmss"); Calendar calendar =
				 * Calendar.getInstance();
				 * calendar.setTimeInMillis(newStartDate.getTime());
				 * calendar.add(Calendar.HOUR, -1); calendar.getTime(); Date
				 * statisStartTime = calendar.getTime();
				 * calendar.add(Calendar.MINUTE, 59); Date statisEndTime =
				 * calendar.getTime();
				 * 
				 * JobPersistence actionPer = new JobPersistence();
				 * actionPer.setId(Long.parseLong(currentDate+startHour+
				 * startMinute)*10000+jobDetail.getId());//update action id
				 * actionPer.setToJobId(jobDetail.getId());
				 * actionPer.setAuto(jobDetail.getAuto());
				 * actionPer.setConfigs(jobDetail.getConfigs());
				 * actionPer.setCronExpression(jobDetail.getCronExpression());
				 ...
				 * actionPer.setStatus(jobDetail.getStatus());
				 * actionPer.setTimezone(jobDetail.getTimezone());
				 * actionPer.setHostGroupId(jobDetail.getHostGroupId()); try {
				 * System.out.println("周期任务（时）JobId: " + jobDetail.getId()+
				 * ";  ActionId: " +actionPer.getId());
				 * //if(actionPer.getId()>Long.parseLong(currentDateStr)){
				 * context.getGroupManager().saveJob(actionPer);
				 * System.out.println("success");
				 ...
				 * 
				 * } catch (ZeusException e) { System.out.println("failed");
				 * log.error("周期任务（时）JobId:" + jobDetail.getId() + " 生成Action"
				 * +actionPer.getId() + "失败", e); } } } } }catch(Exception ex){
	...
	public void runDependencesJobToAction(List<JobPersistenceOld> jobDetails, Map<Long, JobPersistence> actionDetails,
			String currentDateStr, int loopCount) {
					...
					if (jobDependencies != null && jobDependencies.trim().length() > 0) {
							...
							if (loopCount > 20) {
								if (!jobDetail.getConfigs().contains("sameday")) {
									if (dependActionList.get(deps).size() == 0) {
										List<JobPersistence> lastJobActions = context.getGroupManager()
												.getLastJobAction(deps);
						...
						for (String deps : dependStrs) {
							...
							if (dependActionList.get(deps).size() > dependActionList.get(actionMostDeps).size()) {
								actionMostDeps = deps;
							} else if (dependActionList.get(deps).size() == dependActionList.get(actionMostDeps)
									.size()) {
								if (dependActionList.get(deps).get(0).getId() < dependActionList.get(actionMostDeps)
										.get(0).getId()) {
									actionMostDeps = deps;
								}
							}
							...
							if (actions != null && actions.size() > 0) {
								for (JobPersistence actionModel : actions) {
									...
									for (String deps : dependStrs) {
										if (!deps.equals(actionMostDeps)) {
											...
											for (JobPersistence actionOtherModel : actionOthers) {
												if (Math.abs((actionOtherModel.getId() - actionModel.getId())) < Math
														.abs((actionOtherId - actionModel.getId()))) {
											...
											actionDependencies += String.valueOf(
													(actionOtherId / 1000000) * 1000000 + Long.parseLong(deps));
										}
									}
									// 保存多版本的action
									JobPersistence actionPer = new JobPersistence();
									actionPer.setId((actionModel.getId() / 1000000) * 1000000 + jobDetail.getId());// update
																													// action
																													// id
									actionPer.setToJobId(jobDetail.getId());
									actionPer.setAuto(jobDetail.getAuto());
									...
									try {
										...
										log.error("依赖任务JobId:" + jobDetail.getId() + " 生成Action" + actionPer.getId()
												+ "失败", e);
									}
								}
							}
			...
			if ((jobDetail.getScheduleType() != null && jobDetail.getScheduleType() == 1)
				...
				try {
					...
					if (jobDependencies != null && jobDependencies.trim().length() > 0) {
						...
						//生成action表id编号
						 String str = DateUtil.getToday2().concat(ZeusStringUtil.getRandomNum());
						 Long id = Long.parseLong(str);
						 
						actionPer.setId((id / 1000000) * 1000000 + jobDetail.getId());// update
																										// action
																										// id
						actionPer.setToJobId(jobDetail.getId());
						actionPer.setAuto(jobDetail.getAuto());
	 ...
	 * @param currentDateStr
	 */
	public void runScheduleJobToAction2(List<JobPersistenceOld> jobDetails, Date now, SimpleDateFormat dfDate,
			Map<Long, JobPersistence> actionDetails, String currentDateStr) {
		for (JobPersistenceOld jobDetail : jobDetails) {
			// ScheduleType: 0 独立任务; 1依赖任务; 2周期任务
			if (jobDetail.getScheduleType() != null && jobDetail.getScheduleType() == 0) {

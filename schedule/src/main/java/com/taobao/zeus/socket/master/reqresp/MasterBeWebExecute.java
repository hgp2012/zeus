...
public class MasterBeWebExecute {
	public WebResponse beWebExecute(MasterContext context,WebRequest req) {
		if(req.getEk()==ExecuteKind.ManualKind || req.getEk()==ExecuteKind.ScheduleKind){
			...
			System.out.println("=======MasterBeWebExecute 手动恢复==============");
			SocketLog.info("=======MasterBeWebExecute 手动恢复==============");
			...
			SocketLog.info("MasterBeWebExecute send web execute response,rid="+req.getRid()+",jobId="+jobId);
			return resp;
			...
			WebResponse resp=WebResponse.newBuilder().setRid(req.getRid()).setOperate(WebOperate.ExecuteJob)
				.setStatus(Status.OK).build();
			SocketLog.info("MasterBeWebExecute send web debug response,rid="+req.getRid()+",debugId="+debugId);
			return resp;
		}
		return null;
	}

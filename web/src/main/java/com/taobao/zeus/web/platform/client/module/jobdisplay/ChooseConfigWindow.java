...
public class ChooseConfigWindow extends Window{
	...
	public ChooseConfigWindow(JobPresenter presenter,int type){
		...
		this.type = type;
		this.jobPresenter=presenter;
		if(type==3){//自定义新增   指定任务及依赖任务执行     3表示执行当前任务和后面所有依赖的任务
			refresh_dep();
		}else{
			refresh();
		}
	...
	private void refresh_dep(){
		if(jobPresenter!=null){
			RPCS.getJobService().run(jobPresenter.getJobModel().getId(), type, new AbstractAsyncCallback<Void>() {
				@Override
				public void onSuccess(Void result) {

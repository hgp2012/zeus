...
import com.taobao.zeus.web.platform.client.module.jobdisplay.ChooseVersionWindow;
...
public class CardInfo extends CenterTemplate implements Refreshable<JobModel>{
	...
	private TextButton dependences=new TextButton("执行依赖",new SelectHandler() {		
		@Override
		public void onSelect(SelectEvent event) {
			ConfirmMessageBox box=new ConfirmMessageBox("依赖任务", "你确认执行此任务和依赖的所有任务?");
			box.addHideHandler(new HideHandler() {
				@Override
				public void onHide(HideEvent event) {
					Dialog btn = (Dialog) event.getSource();
					if(btn.getHideButton().getText().equalsIgnoreCase("yes")){
						new ChooseConfigWindow(presenter,3);
					}
				}
			});
			box.show();			
	...
	//自定义新增  多版本页面
	private TextButton jobVersion=new TextButton("以前版本",new SelectHandler() {
		@Override
		public void onSelect(SelectEvent event) {
			new ChooseVersionWindow(presenter,1);			
		}
	});
	
	//自定义 新加依赖匹配
	private TextButton dependent=new TextButton("依赖匹配",new SelectHandler() {
		@Override
		public void onSelect(SelectEvent event) {
			Boolean auto=presenter.getJobModel().getAuto();
			RPCS.getJobService().autoDependent(presenter.getJobModel().getId(), new AbstractAsyncCallback<String>() {
				@Override
				public void onSuccess(String result) {
					final ProgressMessageBox box=new ProgressMessageBox("自动匹配", "正在自动匹配");
					...
					if (result.equals("success")) {
						Info.display("成功", "匹配成功");
					}else {
						Window.alert(result+" 没有匹配成功");
					}
...
//	private FieldLabel host;
	...
	public CardInfo(JobPresenter presenter){
		...
		addButton(toDoc);
		addButton(jobVersion);//自定义新增   多版本页面
		addButton(dependent); //自定义新增 自动匹配依赖
		addButton(dependences); //自定义新增 自动匹配依赖

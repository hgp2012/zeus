...
public class ChooseVersionWindow extends Window{
...

	{
		...
		private ListStore<Map<String, String>> store=new ListStore<Map<String, String>>(new ModelKeyProvider<Map<String, String>>() {
			public String getKey(Map<String, String> item) {
				return item.get("name")+"("+item.get("actionid")+")";
			}
		});
			...
			public void onSelect(SelectEvent event) {
				...
				if(combo.validate() && md!=null){
					if(jobPresenter!=null){
						...
						RPCS.getJobService().updateVersion(actionid, jobPresenter.getJobModel().getId(), new AbstractAsyncCallback<JobModel>() {
							@Override
		...
		{
			win.setHeadingText("选择要恢复的JOB版本");
			win.setSize("350", "100");
			win.setModal(true);
	...
	public ChooseVersionWindow(JobPresenter presenter){
		this();
		this.jobPresenter=presenter;
		refresh();
	}
	public ChooseVersionWindow(JobPresenter presenter,int type){
		this();
		this.type = type;
		this.jobPresenter=presenter;
		refresh();
	...
	public ChooseVersionWindow() {
		...
		combo.setTriggerAction(TriggerAction.QUERY);
		combo.setStore(store);
		add(new FieldLabel(combo, "选择要恢复的JOB版本"));
		addButton(submit);
		// TODO Auto-generated constructor stub
	}
	private void refresh(){
		if(jobPresenter!=null){
			RPCS.getJobService().getJobVersion(jobPresenter.getJobModel().getId(), new AbstractAsyncCallback<List<String>>() {
				public void onSuccess(List<String> result) {
					if(result==null || result.size()==0){
						...
						while (itr.hasNext()) {
						    ...
						    String str = (String) nextObj.toString();
